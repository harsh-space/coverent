  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`w-full bg-ui-white border-2 rounded-xl px-5 py-4 flex items-center justify-between transition-all duration-300 ${
          isOpen ? "border-brand-yellow shadow-lg shadow-brand-yellow/10" : "border-ui-gray-light hover:border-ui-gray-dark/30"
        }`}
      >
            ...
            {options.map((option, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-5 py-3 text-sm font-bold transition-all flex items-center justify-between group ${
                    isSelected ? "bg-brand-yellow/10 text-ui-black" : "hover:bg-ui-gray-light/30 text-ui-gray-dark hover:text-ui-black"
                  }`}
                >
  ...
  return (
    <div className="flex flex-col gap-8 pb-10 text-ui-black font-sans">
      <div className="flex flex-col mb-2">
        ...
        <p className="text-sm font-bold text-ui-gray-dark italic">Simulation Control Center</p>
      </div>

      <div className="bg-ui-white border border-ui-gray-light rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row min-h-[600px]">
        {/* Tab Sidebar */}
        <div className="w-full md:w-72 border-r border-ui-gray-light bg-ui-gray-light/10 p-6 flex flex-col gap-3">
          <button
            onClick={() => setActiveTab("single")}
            className={`flex items-center gap-4 px-5 py-5 rounded-2xl transition-all font-black text-sm tracking-wide ${
              activeTab === "single" ? "bg-brand-yellow text-ui-black shadow-lg" : "hover:bg-ui-gray-light/40 text-ui-gray-dark"
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-4 px-5 py-5 rounded-2xl transition-all font-black text-sm tracking-wide ${
              activeTab === "stress" ? "bg-brand-yellow text-ui-black shadow-lg" : "hover:bg-ui-gray-light/40 text-ui-gray-dark"
            }`}
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="max-w-xl flex flex-col gap-10">
                 ...
                 <div className="flex bg-ui-gray-light/30 p-2 rounded-2xl gap-2">
                    {INTENSITIES.map(level => (
                      <button
                        ...
                        className={`flex-1 py-4 text-[11px] font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${
                          selectedIntensity === level ? "bg-ui-white text-ui-black shadow-lg" : "text-ui-gray-dark hover:text-ui-black"
                        }`}
                      >
                        {level}
                      </button>
               ...
               <div className="pt-6 relative">
                 <button 
                   ...
                   disabled={isLoading}
                   className={`btn-primary w-full py-5 text-lg shadow-2xl shadow-brand-yellow/40 flex gap-3 transition-all ${isLoading ? "opacity-50 grayscale cursor-not-allowed" : "hover:translate-y-[-2px] hover:scale-[1.01]"}`}
                 >
                   {isLoading ? (
            ...
            <div className="max-w-3xl flex flex-col gap-8">
               ...
               <div className="mt-4">
                 <button 
                   ...
                   className={`btn-primary w-full py-5 text-lg tracking-wider ${isStressTesting ? "opacity-50 cursor-not-allowed shadow-none" : "shadow-2xl shadow-brand-yellow/40 hover:translate-y-[-2px] active:scale-[0.98]"}`}
                 >
